...
async def test_usecases_create_should_return_unprocessable_entity():
    with pytest.raises(UnprocessableEntityException) as err:
        await product_usecase.create(body="")

    assert err.value.message == "Invalid or incomplete information"
...
async def test_usecases_update_should_return_success(product_up, product_inserted):
    ...
    result = await product_usecase.patch(id=product_inserted.id, body=product_up)
